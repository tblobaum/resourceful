...
Resource._request = function (/* method, [key, obj], callback */) {
  ...
  this.runBeforeHooks(method, obj, callback, function () {
    ...
    if (!that.connection[method]) {
      args.pop()(new Error(method +' method not found on protocol ' + that.connection.protocol))
    }
    else { 
      if (!args) args = []
      args[args.length] = args[args.length] || that.resource
...
Resource.get = function (id, callback) {
  ...
  return id ?
    this._request.call(this, 'get', id, callback)
...
Resource.reload = function (key, callback) {
  return this._request("reload", key, callback);
};
...
Resource.filter = function (name, options, filter) {
  ...
  var params = { options: options, filter: filter, resource: this }
  return this._request("filter", name, params, function () {});
};
...
Resource.prototype.reload = function (callback) {
  return this.constructor.reload(this._id, callback);
};
